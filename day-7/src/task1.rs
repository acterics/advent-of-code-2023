use crate::common::{Hand, HandType, compare_cards};
...
pub fn solution(filename: &str) -> i32 {
    ...
    let hands_iterator = file_content.split('\n')
        .map(|line| {
            Task1Hand { hand: Hand::parse_hand(line, get_hand_type) }
        });
...
impl<'a> PartialOrd for Task1Hand<'a> {
